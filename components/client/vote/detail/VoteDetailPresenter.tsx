...
export function VoteDetailPresenter({ 
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 text-xs sm:text-sm font-mono font-bold">
          {days > 0 && (
            <>
              <span className="bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded text-xs">{days}일</span>
              <span className="text-gray-400">:</span>
            </>
          )}
          <span className="bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded text-xs">{hours.toString().padStart(2, '0')}시</span>
          <span className="text-gray-400">:</span>
          <span className="bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded text-xs">{minutes.toString().padStart(2, '0')}분</span>
          <span className="text-gray-400">:</span>
          <span className="bg-red-100 text-red-800 px-1.5 py-0.5 rounded animate-pulse text-xs">{seconds.toString().padStart(2, '0')}초</span>
        </div>
      </div>
    );
  };
  ...
  const confirmVote = async () => {
    ...
    try {
      // TODO: 실제 투표 API 호출
      console.log('Voting for:', { voteId: vote.id, itemId: voteCandidate.id, amount: voteAmount });
        ...
        <div className="relative overflow-hidden">
          ...
          <Card className="border-0 bg-white/80 backdrop-blur-sm">
            <Card.Header className="pb-2">
              ...
              {voteContent && (
                <p className="text-gray-600 text-xs md:text-sm leading-relaxed">{voteContent}</p>
              )}
            </Card.Header>
              ...
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-bold text-blue-600">{availableVotes}표</span>
                  </div>
                </div>
                {/* 투표 기간 정보 */}
              ...
              >
                <Card 
                  ...
                  className={`
                    ...
                    ${item.rank <= 3 ? 'ring-2 ring-yellow-400/50 bg-gradient-to-br from-yellow-50/50 to-orange-50/50' : 'bg-gradient-to-br from-white to-blue-50/30'}
                    backdrop-blur-sm
                  `}
                >
                  {/* 순위 배지 */}
                  {item.rank && item.rank <= 10 && (
                    <div className={`
                      ...
                      ${item.rank === 1 ? 'bg-gradient-to-br from-yellow-400 to-yellow-600 animate-pulse' :
                        item.rank === 2 ? 'bg-gradient-to-br from-gray-400 to-gray-600' :
                        item.rank === 3 ? 'bg-gradient-to-br from-amber-400 to-orange-500' :
                    ...
                    <div className="text-center">
                      ...
                      <div className="space-y-0.5">
                        <p className="text-xs sm:text-sm font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                          ...
                          <span className="text-xs text-gray-500 ml-0.5">표</span>
                        </p>
                  ...
                  <div className={`
                    ...
                    ${item.rank <= 3 ? 'from-yellow-400 via-orange-500 to-red-500' : 'from-blue-500 via-purple-500 to-pink-500'}
                  `}></div>
                </Card>
              </div>
            );
          ...
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full mx-4 shadow-2xl transform animate-in zoom-in-95 duration-200">
            ...
            <div className="text-center mb-6">
              <div className="w-20 h-20 mx-auto mb-4 rounded-full overflow-hidden ring-4 ring-blue-100">
                <img 
                  src={voteCandidate.artist?.image ? getCdnImageUrl(voteCandidate.artist.image) : '/images/default-artist.png'}
                  alt={voteCandidate.artist?.name ? getLocalizedString(voteCandidate.artist.name, currentLanguage) : '아티스트'}
                  className="w-full h-full object-cover"
                />
              </div>
              <h3 className="text-lg font-bold text-gray-800 mb-1">
                {voteCandidate.artist?.name ? getLocalizedString(voteCandidate.artist.name, currentLanguage) : '아티스트'}
              </h3>
              {voteCandidate.artist?.artistGroup?.name && (
                <p className="text-sm text-gray-500">
                  {getLocalizedString(voteCandidate.artist.artistGroup.name, currentLanguage)}
                </p>
              )}
              {(() => {
                const rankedItem = rankedVoteItems.find(item => item.id === voteCandidate.id);
                return rankedItem?.rank && (
                  <div className="mt-2 flex items-center justify-center gap-1">
                    {rankedItem.rank <= 3 && (
                      <span className="text-lg">
                        {rankedItem.rank === 1 ? '🥇' : rankedItem.rank === 2 ? '🥈' : '🥉'}
                      </span>
                    )}
            ...
            {/* 확인 메시지 */}
            <div className="text-center mb-6">
              <h2 className="text-xl font-bold text-gray-800 mb-2">투표 확인</h2>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-semibold text-gray-700">투표량</label>
                <span className="text-xs text-gray-500">보유: {availableVotes}표</span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 text-center">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setVoteAmount(Math.min(availableVotes, Math.max(1, value)));
                    }}
                ...
                <button
                  onClick={() => setVoteAmount(Math.min(availableVotes, voteAmount + 1))}
              ...
              <div className="flex gap-2 mt-3">
                {[1, 5, Math.min(10, availableVotes), availableVotes].filter((val, idx, arr) => arr.indexOf(val) === idx && val > 0).map((amount) => (
                  <button
                    key={amount}
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={confirmVote}
                disabled={isVoting || voteAmount <= 0 || voteAmount > availableVotes}
