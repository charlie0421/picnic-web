    ...
    return (
      <div className='space-y-3 mt-4'>
        {topThreeItems.map((item, index) => (
          <div
            key={item.id}
            className='flex items-center space-x-3 bg-white/50 backdrop-blur-sm rounded-lg p-2 shadow-sm hover:shadow-md transition-shadow'
          >
            <div className='relative'>
              <div
                className={`absolute -top-2 -right-2 w-7 h-7 rounded-full flex items-center justify-center text-white text-sm font-bold ${RANK_BADGE_COLORS[index]} transform hover:scale-110 transition-transform`}
              >
                {index + 1}
              </div>
            ...
            <div className='flex-1 min-w-0'>
              <div className='text-sm font-semibold text-gray-800 truncate'>
                {item.artist
                  ? item.artist.name?.ko || item.artist.name || 'Unknown Artist'
                  : 'Unknown Artist'}
              </div>
