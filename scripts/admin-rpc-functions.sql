...
-- 5.1 사용자 보너스 히스토리 조회
CREATE OR REPLACE FUNCTION admin_get_user_bonus_history(
    target_user_id uuid,
    limit_count integer DEFAULT 50
)
RETURNS jsonb AS $$
...
-- 5.2 보너스 수동 지급/회수
CREATE OR REPLACE FUNCTION admin_adjust_user_bonus(
    target_user_id uuid,
    bonus_amount integer,
    reason text,
    expired_days integer DEFAULT 30
)
RETURNS jsonb AS $$
DECLARE
    current_user_id uuid;
...
-- 5.3 보너스 잔액 drift 확인
CREATE OR REPLACE FUNCTION admin_check_bonus_drift(target_user_id uuid)
RETURNS jsonb AS $$
DECLARE
    current_user_id uuid;
    is_user_admin boolean;
...
GRANT EXECUTE ON FUNCTION admin_get_user_bonus_history(uuid, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION admin_adjust_user_bonus(uuid, integer, text, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION admin_check_bonus_drift(uuid) TO authenticated;
...
COMMENT ON FUNCTION admin_get_user_bonus_history IS '관리자용 사용자 보너스 히스토리 조회 (요약 + drift 감지 포함)';
COMMENT ON FUNCTION admin_adjust_user_bonus IS '보너스 수동 지급/회수 (audit_logs 자동 기록)';
COMMENT ON FUNCTION admin_check_bonus_drift IS '보너스 잔액 drift 확인 (프로필 vs 계산값 비교)'; 